# https://github.com/yitong2333/proxy-minging
...
def get_channel_http(channel_url):
    try:
        with requests.post(channel_url) as resp:
            data = resp.text
        url_list = re.findall(
            "https?://[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]", data
        )  # 使用正则表达式查找订阅链接并创建列表
...
if __name__ == "__main__":
    ...
    for channel_url in list_tg:
        ...
        for url in temp_url_list:
            if any(item in url for item in allow_list) and all(
                item not in url for item in deny_list
            ):
                url_list.append(url)
